...
static inline _pmalloc_chunk* _pcalloc_create_chunk(_pmalloc_state* arena, size_t size);
static inline _pmalloc_chunk* _pcalloc_create_bin_chunk(_pmalloc_state* arena, size_t size, size_t binIdx);
static inline _pmalloc_chunk* _pcalloc_create_mmap_chunk(_pmalloc_state* arena, size_t size);
...
void* pcalloc(in size_t size) {
	...
	if (_pmalloc_is_mmap_size(size)) {
		_pmalloc_chunk* chunk = _pcalloc_create_mmap_chunk(arena, size);
		...
		if (count > 0) {
			...
			SecureZeroMemory(_pmalloc_chunk2ptr(chunk), size);
			return _pmalloc_chunk2ptr(chunk);
		}
		else {
			_pmalloc_chunk* chunk = _pcalloc_create_bin_chunk(arena, size, binIdx);
			return _pmalloc_chunk2ptr(chunk);
		}
	}
}
...
static inline _pmalloc_chunk* _pcalloc_create_mmap_chunk(_pmalloc_state* arena, size_t size) {
	_pmalloc_chunk* chunk = _pcalloc_create_chunk(arena, size);
	chunk->arena = arena;
